...
class HMC(object):
    ...
    def set_debug(self, on_off):
        ''' Set the internal debug to True or False then this library get a lot more verbose 
            ...
            Returns: nothing
            example hmc.set_debug(True) or hmc_topo.debug(False) '''
        if on_off == True:
    ...
    def get_filenames_ssp(self, SSPid, SSP):
        ...
        return self.get_filenames(url, xxx, type)
    ...
    def get_filenames_NetworkBridge(self, Atomid, Server):#lw
        '''For networkbridge set the URL and resource name
           and call get_filenames directly returning the result
           Arguments: 1=AtomId also call ServerId 2=Server by name for any error reporting
           results the array of filenames '''
        if self.debug:
            print("DEBUG:get_filenames_server(%s,%s)" % (Atomid, Server))
        url = 'https://' + self.HMCname + ':12443/rest/api/uom/ManagedSystem/' + Atomid + '/NetworkBridge'
        xxx = "Server-" + Server + "-NetworkBridge"
        type = 'application/vnd.ibm.powervm.uom+xml'
        return self.get_filenames(url, xxx, type)

    def get_filenames_VIOS(self, Atomid, VIOS):#lw
        '''For networkbridge set the URL and resource name
           and call get_filenames directly returning the result
           Arguments: 1=AtomId also call ServerId 2=Server by name for any error reporting
           results the array of filenames '''
        if self.debug:
            print("DEBUG:get_filenames_server(%s,%s)" % (Atomid, VIOS))
        url = 'https://' + self.HMCname + ':12443/rest/api/uom/VirtualIOServer/' + Atomid
        xxx = "VIOS-" + VIOS + "-" + Atomid
        type = 'application/vnd.ibm.powervm.uom+xml'
        return self.get_filenames(url, xxx, type)

    def get_filenames_Link(self, Atomid, VIOS):#lw
        '''For networkbridge set the URL and resource name
           and call get_filenames directly returning the result
           Arguments: 1=AtomId also call ServerId 2=Server by name for any error reporting
           results the array of filenames '''
        if self.debug:
            print("DEBUG:get_filenames_server(%s,%s)" % (Atomid, VIOS))
        url = 'https://' + self.HMCname + ':12443/rest/api/uom/VirtualIOServer/' + Atomid + "/LinkAggregation"
        xxx = "Link-" + VIOS + "-" + Atomid
        type = 'application/vnd.ibm.powervm.uom+xml'
        return self.get_filenames(url, xxx, type)

    def get_filenames_Serverprofile(self, Atomid, ServerName):#lw
        '''For networkbridge set the URL and resource name
           and call get_filenames directly returning the result
           Arguments: 1=AtomId also call ServerId 2=Server by name for any error reporting
           results the array of filenames '''
        if self.debug:
            print("DEBUG:get_filenames_server(%s,%s)" % (Atomid, ServerName))
        url = 'https://' + self.HMCname + ':12443/rest/api/uom/ManagedSystem/' + Atomid + '/LogicalPartition'
        xxx = "Profile-" + ServerName + "-" + Atomid
        type = 'application/vnd.ibm.powervm.uom+xml'
        return self.get_filenames(url, xxx, type)

    def get_filenames_LogicalPartitionProfile(self, Atomid,LogicalPartition ):#lw
        '''For networkbridge set the URL and resource name
           and call get_filenames directly returning the result
           Arguments: 1=AtomId also call ServerId 2=Server by name for any error reporting
           results the array of filenames '''
        if self.debug:
            print("DEBUG:get_filenames_server(%s,%s)" % (Atomid, LogicalPartition))
        url = 'https://' + self.HMCname + ':12443/rest/api/uom/LogicalPartition/' + Atomid + '/LogicalPartitionProfile'
        xxx = "LogicalPartitionProfile-" + LogicalPartition + "-" + Atomid
        type = 'application/vnd.ibm.powervm.uom+xml'
        return self.get_filenames(url, xxx, type)



    def get_filenames(self, fileUrl, resource_name,type):
        ...
        result = []
        fileHeaders = {'X-API-Session': self.token, 'Accept': type }
